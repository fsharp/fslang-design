...
The `exn` requirement is checked at the moment the `reraise ()` call is type-checked, so a builder whose handler parameter type is generic or not yet solved at that point also keeps FS0413.

### Known behavioral wrinkle: `when` guards in sequence expressions

Sequence expressions compile their `try ... with` through the `RuntimeHelpers.EnumerateTryWith` runtime helper, which invokes the compiled guard from inside a real CLR exception filter block. The CLR discards exceptions thrown from a filter, so a guard-position `reraise ()` in a `seq { }` does not perform the `ExceptionDispatchInfo` rethrow: the original exception simply continues unwinding (which observationally resembles a rethrow, but appends no rethrow frame). In builder-based computation expressions, guard-position `reraise ()` behaves like any other position.
...
- Under the same lexical rule, `reraise ()` in the `finally` of a `try ... finally` written inside a computation expression handler compiles and rethrows the caught exception, replacing any exception already in flight — a real handler rejects the identical shape with FS0413. Should the `finally` position be excluded?
- Should guard-position `reraise ()` in sequence expressions be an error instead of the filter-swallowed behavior described above?
