...
**Compiler and tooling slow-downs on large interface lists.**  For the specific case of SRTPS, the presence of enormous generic interface lists can cause compiler slow-down. No one ever expected such lists of interfaces in .NET, this is a volcano of hidden complexity lying under the simple type `double` or `decimal`.
